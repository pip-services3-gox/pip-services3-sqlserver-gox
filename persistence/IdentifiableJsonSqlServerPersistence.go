...
func (c *IdentifiableJsonSqlServerPersistence[T, K]) UpdatePartially(ctx context.Context, correlationId string,
	...
	if err == nil {
		...
		c.IdentifiableSqlServerPersistence.Logger.Trace(ctx, correlationId, "Updated partially in %s with id = %s", c.IdentifiableSqlServerPersistence.TableName, id)
		return result, nil
	}
	return result, rows.Err()
}
